...
async function createNotification({ recipient, type, title, message, channels = ['in_app'], data = {} }) {
  ...
  if (user) {
    ...
    if (channels.includes('email') && user.email) await sendEmail({ to: user.email, subject: title, text: message });
    if (channels.includes('sms') && user.mobileNumber) await sendSms(phone, message);
    if (channels.includes('whatsapp') && user.mobileNumber) await sendWhatsApp(phone, message);
  }
